  ...
  try {
    ...
    if (!cart) {
      return res
        ...
        .json({ status: "error", message: "No se encuentra el carrito" });
    }
    let importeTotal = 0;
    const products = [];
    ...
    if (importeTotal === 0) {
      return res
        ...
        .json({
          status: "error",
          message: "El importe a cobrar debe ser mayor que 0.",
        });
    }
    ...
    res
      .status(HTTTP_STATUS_CODES.CREATED)
      .json({ status: "success", payload: result, stripePK });
  } catch (error) {
    console.error(error);
    res
