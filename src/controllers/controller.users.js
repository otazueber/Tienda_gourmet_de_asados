...
router.post('/', async (req, res) => {
    try {            
        ...
        if (user) {
            ...
            return res.status(HTTTP_STATUS_CODES.CONFLICT).json({ status: 'error', message: 'No se pudo dar de alta este usuario' })
        };            
        const newUserInfo = {
            first_name,
            last_name,
        ...
        res.status(HTTTP_STATUS_CODES.OK).json({status: 'success', message: 'Usuario registrado'});
    } catch (error) {
        req.logger.error(error.message);
        res.status(HTTTP_STATUS_CODES.INTERNAL_SERVER_ERROR).json({ status: 'error', error: 'Internal server error' });
    };
}
);
...
router.put('/premium/:uid', async (req, res) => {
    ...
    if (!user) {
        res.status(HTTTP_STATUS_CODES.NOT_FOUND).json({ status: 'error', message: 'Usuario no encontrado' })
    }
    else if (user.role == 'premium') {
        DbUserManager.actualizarRol(user.email, 'user');
        res.status(HTTTP_STATUS_CODES.OK).json({ status: 'success', message: 'Se actualizó el rol del usuario a user' });
        ...
        if ((user.documentStatus.identification) & (user.documentStatus.proofOfAddress) & (user.documentStatus.bankStatement)) {
            ...
            res.status(HTTTP_STATUS_CODES.OK).json({ status: 'success', message: 'Se actualizó el rol del usuario a premium' });
        } else {
            res.status(HTTTP_STATUS_CODES.BAD_REQUEST).json({ status: 'error', message: 'No ha terminado de procesar su documentación' });
        }
    }
});
...
router.post('/:uid/documents', uploader.fields([
    ...
    res.status(HTTTP_STATUS_CODES.OK).json({ status: 'success', message: 'documentos guardados' });
});
