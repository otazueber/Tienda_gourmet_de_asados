...
router.get('/', async (req, res) => {
    ...
    try {
        const products = await DbProductManager.getProducts(endPoint, limit, page, sort, query);
...
router.get('/:pid', async (req, res) => {
    ...
    try {
        ...
        if (product) {
            ...
            res.status(HTTTP_STATUS_CODES.NOT_FOUND).json({ error: 'Producto no encontrado' });
        }
    } catch (error) {
        req.logger.error(error.message);
        res.status(HTTTP_STATUS_CODES.INTERNAL_SERVER_ERROR).json({ error: 'Internal server error.' });
    ...
    try {
        ...
        if (product) {
            if ((product.owner == req.user.email) || (req.user.role = 'admin')) {
                ...
                if (result) {
                    res.status(HTTTP_STATUS_CODES.OK).json({ message: 'Producto actualizado satisfactoriamente' });
                }
                else {
                    res.status(HTTTP_STATUS_CODES.INTERNAL_SERVER_ERROR).json({ error: 'Internal server error' });
                }
            } else {
                res.status(HTTTP_STATUS_CODES.BAD_REQUEST).json({ status: 'error', message: 'No tiene permisos para eliminar este producto' });
            }
        } else {
            res.status(HTTTP_STATUS_CODES.NOT_FOUND).json({ error: 'Producto no encontrado' });
        }
...
router.post('/insertMany', async (req, res) => {
    try {
        ...
        res.status(HTTTP_STATUS_CODES.OK).json({ status: 'success', message: 'Productos agregados!!!' });
    } catch (error) {
        res.status(HTTTP_STATUS_CODES.INTERNAL_SERVER_ERROR).json({ status: 'error', message: error.message });
    }
});
